...
----
include::{sample-dir}/src/test/java/de/spricom/dessert/sample/DuplicatesTest.java[tags=duplicates]
----
...
all module-info classes is `slice("module-info")`, because it filters by name.
...
with the key assigned. There is another `partitionBy` method with a second `PartitionSliceFactory`

...
When using dessert you work most of the time with some `Slice` implementation.
...
Classpath::
The `Classpath` is the starting point of every _dessert_ test and the biggest
possible slice.
...
The module API is fully supported for JDK 8 and earlier. Therefore, dessert comes `FixedModule`

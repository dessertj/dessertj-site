...
For this purpose you can define an instances variable that you can use all over your
architecture test:
...
indicator for design flaws. There is always a clean design that without package-cycles.
Use the following code to assert that some building block `block` has no package-cycles:
...
Big applications, sometimes called _monolith_, are used be different stakeholders for
very different use-cases. Image some stamp-shop where a customer can buy stamps. The shop-owner
mast be able to add new offerings and update prices. An office clerk handles the orders

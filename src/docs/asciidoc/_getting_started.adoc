:sample-dir: C:\Code\git\dessert\dessert-site\dessert-sample

== Getting Started

Add the dependency to your project:

[source, xml, indent=0]
----
include::{sample-dir}/pom.xml[tags=dep]
----

If you wan't to try the most current snapshot version add specify the repository:

[source, xml, indent=0]
----
include::{sample-dir}/pom.xml[tags=repo]
----

A first dependency test:

[source, java, indent=0]
----
include::{sample-dir}/src/test/java/de/spricom/dessert/DessertSampleTest.java[tags=fails]
----

Each _dessert_ test starts with the `Classpath`. Dessert is all about slicing down the `Classpath` and checking the dependencies between the slices. The `Classpath` is the whole cake. A `Clazz` represents a single .class file, the smalles possible `Slice`. A `Root` represents a classes directory, a .jar file or a module. All of them are slices, hence they implement the `Slice` interface. `SliceAssertion` provides a static `assertThat` method to check for unwanted dependencies between slices.

The test above will fail, because it has a dependency to the junit-jupiter-api.jar. Thus it produces the
following output:

----
java.lang.AssertionError: Illegal Dependencies:
de.spricom.dessert.DessertSampleTest
 -> de.spricom.dessert.assertions.SliceAssert
 -> de.spricom.dessert.assertions.SliceAssertions
 -> de.spricom.dessert.slicing.Classpath
 -> de.spricom.dessert.slicing.Clazz
 -> de.spricom.dessert.slicing.Root
 -> de.spricom.dessert.slicing.Slice
 -> java.lang.Class
 -> java.lang.Object
 -> java.lang.String
 -> org.junit.jupiter.api.Test
----

The following test shows some other methods to get slices from the `Classpath`:

[source, java, indent=0]
----
include::{sample-dir}/src/test/java/de/spricom/dessert/DessertSampleTest.java[tags=succeeds]
----

The _java..*_ slice represents all classes in the `java` package or any nested package.
The `dessert` method is an alias for `assertThat` to prevent name collisions with other
libraries. It stands for _dependency assert that_. All the assertion methods of
`SliceAssertions` accept more than one slice, like `usesOnly`. They treat the slices as an
union of all the slices passed.
